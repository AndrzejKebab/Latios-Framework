...
namespace Latios.Transforms
{
    ...
    public unsafe struct TransformReadAspect : IJobEach.IParameter
    {
        ...
        /// <summary>
        /// Retrieves the EntityInHierarchyHandle of this TransformReadAspect. Note that if the entity
        ...
        /// <summary>
        /// Retrieves the TransformReadAspect for the specified handle belonging to the same hierarchy
        /// as this TransformReadAspect. When safety checks exist, this method throws if the specifed
        /// handle comes from another hierarchy or if either its handle or this TransformReadAspect's
        /// handle is null.
        /// </summary>
        public TransformReadAspect this[EntityInHierarchyHandle otherHandle]
        {
            get
            {
                ...
                {
                    case AccessType.EntityManager:
                        result.m_worldTransform = ((EntityManager*)m_access)->GetComponentDataRO<WorldTransform>(otherHandle.entity);
                        break;
                    case AccessType.ComponentBroker:
                        result.m_worldTransform = ((ComponentBroker*)m_access)->GetRO<WorldTransform>(otherHandle.entity);
                        break;
                    case AccessType.ComponentBrokerKeyed:
                        result.m_worldTransform = ((ComponentBroker*)m_access)->GetROIgnoreParallelSafety<WorldTransform>(otherHandle.entity);
                        break;
                    case AccessType.ComponentLookup:
                        result.m_worldTransform = ((ComponentLookup<WorldTransform>*)m_access)->GetRefRO(otherHandle.entity);
                        break;
                    default:
        ...
        /// <summary>
        /// Attempts to retrieve the TransformReadAspect for the specified handle belonging to the same
        /// hierarchy as this TransformReadAspect. When safety checks exist, this method throws if the
        /// specifed handle comes from another hierarchy. This method returns false if the transform
        /// is not present.
        /// </summary>
        public bool TryGetAspect(in EntityInHierarchyHandle otherHandle, out TransformReadAspect otherTransformReadAspect)
        {
            CheckBelongsToSameHierarchy(in otherHandle);
            var result      = this;
            result.m_handle = otherHandle;
            switch (m_accessType)
            {
                ...
                case AccessType.ComponentBrokerKeyed:
                    ...
                    break;
                case AccessType.ComponentLookup:
                    ((ComponentLookup<WorldTransform>*)m_access)->TryGetRefRO(otherHandle.entity, out result.m_worldTransform);
                    break;
                default:
                    result.m_worldTransform = default;
                    break;
            }
            otherTransformReadAspect = result;
        ...
        void CheckBelongsToSameHierarchy(in EntityInHierarchyHandle otherHandle)
        {
            if (m_handle.isNull || otherHandle.isNull || m_handle.m_hierarchy != otherHandle.m_hierarchy)
                throw new System.ArgumentException("The EntityInHierarchyHandle does not belong to the same hierarchy as this TransformReadAspect.");
        }

        [Conditional("ENABLE_UNITY_COLLECTIONS_CHECKS"), Conditional("UNITY_DOTS_DEBUG")]
        void CheckWorldTransformIsValid(in RefRO<WorldTransform> transform)
        {
